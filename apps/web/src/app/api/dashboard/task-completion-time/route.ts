...
interface TaskCompletionTimeResult extends RowDataPacket {
  AgeRange: string;
...
export async function GET(request: NextRequest) {
  try {
    ...
    // שאילתא לחישוב זמן השלמת משימות לפי טווח גילאים (עם פילטור לפי רמת אנגלית)
    const taskCompletionTimeQuery = `
      SELECT 
        u.AgeRange,
        AVG(t.DurationTask / 60) as avg_duration_minutes,
        COUNT(t.TaskId) as task_count,
        COUNT(DISTINCT u.UserId) as user_count
      FROM Tasks t
      JOIN Users u ON t.UserId = u.UserId
      WHERE t.CompletionDate IS NOT NULL 
        AND t.DurationTask IS NOT NULL 
        AND t.DurationTask > 0
        AND u.IsActive = 1
        ${whereClause}
      GROUP BY u.AgeRange
      ORDER BY 
    ...
    // ביצוע השאילתות
    const [completionTimeRes, taskTypesRes, englishLevelsRes] =
      await Promise.all([
        executeQuery(
          taskCompletionTimeQuery,
          queryParams,
          "Fetch task completion times by age range"
        ),
    ...
    const completionTimeData = completionTimeRes.result as RowDataPacket[];
    ...
    const processedData = completionTimeData.map((row: any) => ({
      ageRange: row.AgeRange,
      avgDurationMinutes: Math.round(row.avg_duration_minutes * 100) / 100,
    ...
    const response = {
      ...
      totalRecords: completionTimeData.length,
    };

    return NextResponse.json(response);
  } catch (error) {
    console.error("Error fetching task completion time data:", error);
